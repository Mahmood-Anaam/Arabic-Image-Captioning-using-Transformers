...
def train(args, train_dataloader, val_dataset, model, tokenizer, valloss_dataloader=None):
    if args.distributed:
    ...
    global_valstep, global_valloss, global_valacc = 0,  0.0, 0.0
    model.zero_grad()
    eval_log = []
    best_score = 0
    for epoch in range(int(args.num_train_epochs)):
        for step, (img_keys, batch) in enumerate(train_dataloader):
            ...
            if not args.scst:
              # tells your model that you are training the model, affects layers like dropout, batchnorm etc.
                model.train()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    logger.info("Training {{\"Epoch\": {}, \"global_step\": {}, \"lr\": {:.6f}, \"loss\": {:.4f}, " \
                        "\"global_loss\": {:.4f}, \"score\": {:.4f}, \"global_score\": {:.4f}}}".format(epoch, global_step, 
        ...
        logger.info("Training {{\"Epoch\": {}, \"global_step\": {}, \"lr\": {:.6f}, \"global_loss\": {:.4f}, " \
                    "\"global_score\": {:.4f}}}".format(epoch, global_step, optimizer.param_groups[0]["lr"], 
                    global_loss / global_step, global_acc / global_step))

        # Calculate and output validation loss
        if args.evaluate_during_training:
            for step, (img_keys, batch) in enumerate(valloss_dataloader):
                batch = tuple(t.to(args.device) for t in batch)
                # model.eval()
                inputs = {'input_ids': batch[0], 'attention_mask': batch[1],
                    'token_type_ids': batch[2], 'img_feats': batch[3], 
                ...
                if args.gradient_accumulation_steps > 1:
                    loss = loss / args.gradient_accumulation_steps
                global_valloss += loss.item()
                global_valacc += batch_acc
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    global_valstep += 1
            logger.info("Validation {{\"Epoch\": {}, \"global_step\": {}, \"global_loss\": {:.4f}, " \
                        "\"global_score\": {:.4f}}}".format(epoch, global_valstep, global_valloss / global_valstep, 
                        global_valacc / global_valstep))

        # Save and evaluate
        if (args.save_steps > 0 and (epoch+1) % args.save_steps == 0) or global_step == t_total:
            checkpoint_dir = save_checkpoint(model, tokenizer, args, epoch, global_step)
            '''
            if args.evaluate_during_training: 
                logger.info("Perform evaluation at step: %d" % (global_step))
...
def main():
    ...
    if args.do_train:
        ...
        val_dataloader = None
        valloss_dataloader = None
        if args.evaluate_during_training:
            val_dataloader = make_data_loader(args, args.val_yaml, tokenizer,
                args.distributed, is_train=False)
            valloss_dataloader = make_data_loader(args, args.val_yaml, tokenizer,
                args.distributed, is_train=True, is_valloss=True)
        last_checkpoint = train(args, train_dataloader, val_dataloader, model, tokenizer, valloss_dataloader=valloss_dataloader)
